...
class F_conv(nn.Module):
    ...
    def __init__(self, in_channels, channels, channels_hidden=c.channel_hidden,
                 kernel_size=3, leaky_slope=0.1,
                 batch_norm=False):
        super(F_conv, self).__init__()

        if not channels_hidden:
            channels_hidden = in_channels
        ...
        self.pos_emb = nn.Conv2d(in_channels, in_channels, kernel_size=7, padding=3, groups=in_channels)
...
class ReversibleGraphNet(nn.Module):
    ...
    def __init__(self, node_list, ind_in=None, ind_out=None, verbose=False, n_jac=1):
        ...
        # o(n.id,(input.id,input.out0),(self.id,self.output0))
