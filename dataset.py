...
class Btad_Dataset(Dataset):
    def __init__(self, c, is_train=True):
        assert c.class_name in BTAD_CLASS_NAMES, 'class_name: {}, should be in {}'.format(c.class_name,
                                                                                          BTAD_CLASS_NAMES)
        self.dataset_path = c.dataset_path
        self.class_name = c.class_name
        self.is_train = is_train
        ...
        if is_train:
            self.transform_x = T.Compose([
                T.Resize(c.img_size, InterpolationMode.LANCZOS),
                T.RandomRotation(5),
                T.CenterCrop(c.crop_size),
                T.ToTensor()])
        # test:
        ...
        # mask
        self.transform_mask = T.Compose([
            T.Resize(c.crop_size, InterpolationMode.NEAREST),
            T.ToTensor()])
